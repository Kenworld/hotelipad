...
purchase_btns.forEach((btn) => {
  btn.addEventListener("click", (e) => {
    ...
    const invoice_wrapper =
      e.currentTarget.parentNode.querySelector(".invoice-option");
...
function priceslider() {
  if ($("#slider-tooltips").length > 0) {
    ...
    tooltipSlider.noUiSlider.on("update", function (values, handle, unencoded) {
      formatValues[0].innerHTML = "Price: " + "GHS " + values[0];
      formatValues[1].innerHTML = "GHS " + values[1];
    });
  }
}
priceslider();
